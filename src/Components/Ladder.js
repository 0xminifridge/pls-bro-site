...
export default function Ladder({
  ...
  const mintPublic = async (amount, price) => {
  ...
  const mintWhitelist = async (amount, price) => {
    setTransactionProcessing(true);
    try {
      const { ethereum } = window;
      ...
      if (ethereum) {
        ...
        if (LADDER_ADDRESS && LADDER_ABI && signer) {
          ...
          let tx = await contract.mintAllowlist(amount, {
            value: ethers.utils.parseEther(totalPrice.toString()),
          });
          let hash = tx.hash;
          setTransactionHash(hash);
  ...
  const mintPressed = async () => {
    if (whitelisted) {
      await mintWhitelist(mintAmount, 2.2);
    } else {
      await mintPublic(mintAmount, 3);
    }
  };
      ...
      <div class="text-white flex justify-center flex-col w-full h-full">
        <div class="m-auto flex flex-col justify-center">
          ...
          <h1 class="text-3xl md:text-7xl m-auto p-2 font-bold hover:text-gray-400">
            <a href="https://twitter.com/TheLadder_" target="_blank">
              The Ladder
            </a>
          </h1>
          ...
          <div class="flex justify-center flex-row text-gray-400">
            <span class="mx-2 my-1 hover:text-white">
              <a
                href="https://twitter.com/TheLadder_"
                target="_blank"
            ...
            <span class="mx-2 my-1 hover:text-white">
              <a href="https://discord.com/invite/sD2A5mjuuH" target="_blank">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 71 55"
            ...
            <span class="mx-2 my-1 hover:text-white">
              <a href="https://theladdergame.com/" target="_blank">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
          ...
          {/* <p class="text-xl m-auto text-gray-600">
            {"<"}Answer questions correctly to ascend the Ladder and collect
            prizes{"/>"}
          </p> */}
          <a
